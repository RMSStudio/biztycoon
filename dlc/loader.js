  ...
  const REGISTRY = [
    {
      ...
      icon:    '⚡',
      version: '0.2',
      desc:    'Стартовые перки-руны (выбор 1 из 3 на старте) + сюжетные арки (многоэтапные цепочки событий по триггерам).',
      status:  'wip',
      // Реализация механик физически живёт в src/runes.js и src/storyarcs.js
      // (встраивается в single-HTML билд), но гейтуется проверкой включённости
      // этого DLC через localStorage. Файл ниже — координатор/индикатор.
