...
const DLC = (() => {
  ...
  const REGISTRY = [
    ...
    },
    {
      ...
      icon:    '🌐',
      version: '0.2',
      desc:    'Открытие механик между ранами (Ф.7): первый ран — голый костяк, каждый ран открывает новые системы (найм/скаутинг/рынок/M&A) за экспертизу. Ранние раны почти непроходимы, с полностью собранной игрой — проходимы.',
      status:  'wip',
      // Реальная логика — src/unlocks.js (реестр модулей + гейт isModuleUnlocked +
      // экономика экспертизы §15) и src/unlocks-ui.js (петля рана + экран
      // «Дерево открытий»). Оба в src/ — dist работает без папки dlc/.
      // Файл ниже — координатор/маркер.
      scripts: ['dlc/unlocks/unlocks.js'],
      styles:  [],
    },
    {
      id:      'strategy',
