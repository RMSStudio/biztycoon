...
const DLC = (() => {
  ...
  const REGISTRY = [
    ...
    },
    {
      ...
      icon:    '🌐',
      version: '0.1',
      desc:    'Открытие механик между ранами (Ф.7): первый ран — голый костяк, каждый ран открывает новые системы (найм/скаутинг/рынок/M&A). Ранние раны почти непроходимы, с полностью собранной игрой — проходимы.',
      status:  'wip',
      // Реальная логика — src/unlocks.js (реестр модулей + гейт isModuleUnlocked),
      // грузится как <script> и читает localStorage живьём. Файл ниже — координатор/маркер.
