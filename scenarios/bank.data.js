...
window.SCENARIO_DATA = {
  ...
  "runMap": {
    "stages": [
      { "id": "bank_license",  "name": "Получили лицензию", "icon": "📜", "color": "#94a3b8", "monthEnd":  6,
        "sub": "Документы получены, первые расчётные счета открыты. Учимся не нарушить ни одно ЦБ-предписание." },
      { "id": "bank_retail",   "name": "Розничная сеть",    "icon": "🏦", "color": "#22d3ee", "monthEnd": 14,
        "sub": "Запускаем отделения и приложение. Депозитная база растёт, нужно держать норматив Н2." },
      { "id": "bank_corp",     "name": "Корпоративный портфель", "icon": "🏢","color":"#a78bfa","monthEnd":22,
        "sub": "Заходят средние и крупные компании. Один дефолт может выкосить квартал — строим скоринг." },
      { "id": "bank_private",  "name": "Private banking",   "icon": "💎", "color": "#f59e0b", "monthEnd": 30,
        "sub": "Состоятельные клиенты, индивидуальные продукты. Маржа высокая, KPI по оценке клиента — критичный." },
      { "id": "bank_topten",   "name": "Топ-10",            "icon": "👑", "color": "#facc15", "monthEnd": null,
        "sub": "Системно значимые. Цели по капиталу, M&A на горизонте. Финальный рывок к IPO/выкупу." }
    ],
    // ──────────────────────────────────────────────────────────────────
    // v3.25 (2026-06-15): балансовая дифференциация banking-runMap
    //
    // Универсальные бонусы перевзвешены под банковский профиль (overhead
    // 85K vs 35K у агентства, target 9M vs 7.5M, бóльшие сделки T-гейтов).
    // Базовая логика: overhead давит сильнее (×2.4 в абсолютных K), маржа
    // и предоплаты на больших сделках значат больше, портфолио-гейты
    // тяжелее, рост репутации до 80+ требуется к корпорату/private.
    //
    // Сравнение agency → bank (только то, что изменено):
    //   deposit_base    money    250K  → 400K   (масштаб банка крупнее)
    //   trust           rep      +5    → +6     (рейтинги бьют по гейтам)
    //   margin          payout%  +5    → +6     (маржа на больших сделках)
    //   automation      overhead −10   → −12    (overhead доминирует)
    //   scoring         caseQ    +5    → +6     (точность одобрений)
    //   legal_team      prepay%  +15   → +20    (корп.предоплата дороже)
    //   wellbeing       fatigue  −15/5%→ −18/6% (выгорание выше у банка)
    //   case_studies    portfolio +10  → +12    (гейты T5-T7 жёстче)
    // (branch_network/pr_dept/rpa/regulator оставлены без изменений —
    //  кэп лидов / реп/мес / скорость / penaltyShield не масштабируются)
    // ──────────────────────────────────────────────────────────────────
    "bonuses": [
      { "id": "deposit_base",  "icon": "🐖", "name": "Депозитная база",
        "desc": "Разовая выплата +400 000 ₽ от размещения депозитов",
        "effects": [{ "money": 400000 }],
        "log":  "🐖 Депозитная база: +400 000 ₽" },
      { "id": "trust",         "icon": "⭐", "name": "Доверие клиентов",
        "desc": "+6 репутации — банк рекомендуют",
        "effects": [{ "rep": 6 }],
        "log":  "⭐ Доверие: +6 реп." },
      { "id": "margin",        "icon": "📈", "name": "Маржинальные продукты",
        "desc": "+6% к выплатам по всем сделкам",
        "effects": [{ "gAdd": { "perkPayoutMult": 0.06 } }],
        "log":  "📈 Маржа: выплаты +6%" },
      { "id": "automation",    "icon": "🤖", "name": "Автоматизация бэк-офиса",
        "desc": "−12% overhead до конца партии",
        "effects": [{ "overheadBump": -0.12 }],
        "log":  "🤖 Автоматизация: overhead −12%" },
      { "id": "scoring",       "icon": "🧠", "name": "Скоринг-модель",
        "desc": "+6 к качеству от кейсов навсегда (точность одобрений)",
        "effects": [{ "gAdd": { "caseQBonus": 6 } }],
        "log":  "🧠 Скоринг: +6 Q" },
      { "id": "branch_network","icon": "🏪", "name": "Сетка отделений",
        "desc": "+1 лид при скаутинге сделок",
        "effects": [{ "gAdd": { "caseScoutBonus": 1 } }],
        "log":  "🏪 Сетка: +1 лид" },
      { "id": "pr_dept",       "icon": "📣", "name": "Служба коммуникаций",
        "desc": "+1 восстановление репутации/мес",
        "effects": [{ "gAdd": { "caseRepBonus": 1 } }],
        "log":  "📣 PR: +1 реп/мес" },
      { "id": "rpa",           "icon": "🚀", "name": "RPA-команда",
        "desc": "+5% к скорости обработки сделок",
        "effects": [{ "gAdd": { "speedUpgrades": 0.05 } }],
        "log":  "🚀 RPA: скорость +5%" },
      { "id": "legal_team",    "icon": "💳", "name": "Усиленный юр-блок",
        "desc": "+20% к шансу предоплаты от корпоратов",
        "effects": [{ "gAdd": { "perkPrepayBonus": 0.20 } }],
        "log":  "💳 Юр-блок: предоплата +20%" },
      { "id": "regulator",     "icon": "🛡", "name": "Лобби в регуляторе",
        "desc": "Штраф просрочки бьёт по репутации −50%",
        "effects": [{ "gSet": { "perkPenaltyShield": true } }],
        "log":  "🛡 Лобби: штраф просрочки −50%" },
      { "id": "wellbeing",     "icon": "🌿", "name": "Программа благополучия",
        "desc": "−18 усталости команды, +6% к восст.",
        "effects": [{ "fatigue": -18 }, { "gAdd": { "perkRecoveryBonus": 0.06 } }],
        "log":  "🌿 Wellbeing: усталость −18, восст. +6%" },
      { "id": "case_studies",  "icon": "📚", "name": "Премии за кейсы",
        "desc": "+12 баллов портфолио",
        "effects": [{ "gAdd": { "portfolio": 12 } }],
        "log":  "📚 Премии: +12 портфолио" },

      // ────────────────────────────────────────────────────────────────
      // ЭТАП-ЭКСКЛЮЗИВЫ (v3.17 / v3.19): доступны только на конкретных этапах
      // ────────────────────────────────────────────────────────────────
      // ────────────────────────────────────────────────────────────
      // v3.25: банковские эксклюзивы тоже перевзвешены под профиль.
      // Поздние стадии (private/topten) получают более крупные cash-
      // выплаты и portfolio-бусты — деньги нужны на T5–T7 (бюджеты
      // 18–150M), portfolio гейтит топ-сделки. initial_capital и
      // mobile_app/wealth_advisors оставлены — они уже сбалансированы
      // под свой этап (бесплатных правок не делаем).
      //
      //   gov_contract     money         250K → 350K (раньше деньги)
      //   core_banking     money         400K → 550K (retail на масштаб)
      //   fx_desk          payout%       +7   → +8   (корп.деск)
      //   syndicate_lend.  Q             +5   → +6   (экспертиза)
      //   vip_office       payout%       +10  → +12  (private-маржа)
      //   spo_capital      money         800K → 1.2M (биржевой рывок)
      //   systemic_status  rep           +5   → +6   (статус ЦБ)
      //   mna_acquisition  money+portf.  500K/15→700K/20 (поглощение)
      // ────────────────────────────────────────────────────────────

      // 📜 Получили лицензию
      { "id": "gov_contract",     "icon": "💼", "name": "Госконтракт по обслуживанию",
        "desc": "Разовая выплата +350 000 ₽ · +2 репутации (банк выиграл тендер бюджетников)",
        "stages": ["bank_license"],
        "effects": [{ "money": 350000 }, { "rep": 2 }],
        "log":  "💼 Госконтракт: +350 000 ₽ · +2 реп." },
      { "id": "initial_capital",  "icon": "🪙", "name": "Льготы как новому банку",
        "desc": "−15% overhead до конца партии · +3 репутации (стартовые льготы ЦБ)",
        "stages": ["bank_license"],
        "effects": [{ "overheadBump": -0.15 }, { "rep": 3 }],
        "log":  "🪙 Льготы новичка: overhead −15% · +3 реп." },

      // 🏦 Розничная сеть
      { "id": "core_banking",     "icon": "💾", "name": "Ядро АБС нового поколения",
        "desc": "Разовая выплата +550 000 ₽ от запуска новых сервисов",
        "stages": ["bank_retail"],
        "effects": [{ "money": 550000 }],
        "log":  "💾 Новое АБС: +550 000 ₽" },
      { "id": "mobile_app",       "icon": "📱", "name": "Мобильное приложение в топах",
        "desc": "+6 репутации — банк выходит в топ-3 рейтинга App Store",
        "stages": ["bank_retail"],
        "effects": [{ "rep": 6 }],
        "log":  "📱 Моб.app: +6 реп." },

      // 🏢 Корпоративный портфель
      { "id": "fx_desk",          "icon": "💱", "name": "Валютный деск",
        "desc": "+8% к выплатам — комиссии с FX-операций",
        "stages": ["bank_corp"],
        "effects": [{ "gAdd": { "perkPayoutMult": 0.08 } }],
        "log":  "💱 FX-деск: выплаты +8%" },
      { "id": "syndicate_lending","icon": "🏗", "name": "Синдицированные кредиты",
        "desc": "+1 лид при скаутинге крупных сделок · +6 Q от экспертизы",
        "stages": ["bank_corp"],
        "effects": [{ "gAdd": { "caseScoutBonus": 1, "caseQBonus": 6 } }],
        "log":  "🏗 Синдикаты: +1 лид · +6 Q" },

      // 💎 Private banking
      { "id": "vip_office",       "icon": "🥂", "name": "VIP-офис в центре",
        "desc": "+12% к выплатам со всех сделок (private-маржа)",
        "stages": ["bank_private"],
        "effects": [{ "gAdd": { "perkPayoutMult": 0.12 } }],
        "log":  "🥂 VIP-офис: выплаты +12%" },
      { "id": "wealth_advisors",  "icon": "🧑‍💼", "name": "Команда wealth-advisors",
        "desc": "+10 баллов портфолио · +1 восстановление реп/мес",
        "stages": ["bank_private"],
        "effects": [{ "gAdd": { "portfolio": 10, "caseRepBonus": 1 } }],
        "log":  "🧑‍💼 Wealth-advisors: +10 порт. · +1 реп/мес" },

      // 👑 Топ-10
      { "id": "spo_capital",      "icon": "🏛", "name": "SPO: вышли на биржу",
        "desc": "Разовая выплата +1 200 000 ₽ от размещения акций",
        "stages": ["bank_topten"],
        "effects": [{ "money": 1200000 }],
        "log":  "🏛 SPO: +1 200 000 ₽" },
      { "id": "systemic_status",  "icon": "🌐", "name": "Системно значимый",
        "desc": "Просрочка бьёт по реп −50% (статус ЦБ) · +6 репутации",
        "stages": ["bank_topten"],
        "effects": [{ "gSet": { "perkPenaltyShield": true } }, { "rep": 6 }],
        "log":  "🌐 СЗКО-статус: penalty shield · +6 реп." },
      { "id": "mna_acquisition",  "icon": "🏭", "name": "M&A: поглощение регионального банка",
        "desc": "Разовая выплата +700 000 ₽ · +20 баллов портфолио (расширили базу)",
        "stages": ["bank_topten"],
        "effects": [{ "money": 700000 }, { "gAdd": { "portfolio": 20 } }],
        "log":  "🏭 M&A: +700 000 ₽ · +20 портф." }
    ]
  },

  "milestones": [
    { "id": "first_delivery", "icon": "🏁", "name": "Первая сделка закрыта",
      "desc": "Завершить первую сделку с клиентом",
      "tier": "micro",
      "when": { "deliveriesAtLeast": 1 } },
    { "id": "reputation_50",  "icon": "⭐", "name": "Доверие клиентов",
      "desc": "Поднять репутацию банка до 50",
      "tier": "micro",
      "when": { "reputationAtLeast": 50 } },
    { "id": "first_hire",     "icon": "🤝", "name": "Первая HR-заявка",
      "desc": "Принять в штат первого сотрудника",
      "tier": "micro",
      "when": { "staffAtLeast": 1 } },
    { "id": "first_million",  "icon": "💵", "name": "Первый миллион капитала",
      "desc": "Достичь баланса 1 000 000 ₽",
      "tier": "micro",
      "when": { "moneyAtLeast": 1000000 } },
    { "id": "five_deliveries","icon": "📦", "name": "Пять сделок в портфеле",
      "desc": "Завершить 5 сделок суммарно",
      "tier": "micro",
      "when": { "deliveriesAtLeast": 5 } },
    { "id": "team_8",         "icon": "👥", "name": "Восьмёрка под ключ",
      "desc": "Штат банка вырос до 8 человек",
      "tier": "middle",
      "when": { "staffAtLeast": 8 } },
    { "id": "revenue_7m",     "icon": "📊", "name": "Выручка 7M ₽",
      "desc": "Накопленная выручка преодолела 7 000 000 ₽",
      "tier": "middle",
      "when": { "revenueAtLeast": 7000000 } },
    { "id": "portfolio_30",   "icon": "📚", "name": "Кейсы и обзоры",
      "desc": "Набрать 30 баллов портфолио",
      "tier": "middle",
      "when": { "portfolioAtLeast": 30 } },
    { "id": "original_win",   "icon": "💰", "name": "Старый порог капитала",
      "desc": "Достичь капитала $win (классический win-порог)",
      "tier": "large",
      "when": { "peakMoneyAtLeast": "originalWin" } },
  ...
  "storyArcs": [
    {
      "id": "regulator_call",
      "name": "Звонок из регулятора",
      "trigger": { "minMonth": 4, "maxMonth": 18, "chance": 0.55 },
      "stageDelayMonths": 2,
      "cooldown": 6,
      "stages": [
        {
          ...
          "body": "ЦБ запросил выборку по KYC-процедурам за квартал. Есть пара недокрученных кейсов — рискнуть или подготовиться?",
          "choices": [
            {
              "text": "Поднять документы и доработать (−120 000 ₽)",
              "desc": "Юристы и операционисты на сверхурочке",
              "effects": [
                { "money": -120000 },
                { "fatigue": 6 },
                { "log": ["🏛 Подготовка к ЦБ — документы в порядке", "teal"] }
              ],
              "next": "audit_result"
            },
            {
              "text": "Дать как есть — пройдём",
              "desc": "Без расходов, но шанс на штраф",
              "effects": [
                { "log": ["🏛 Документы поданы «как есть» — ждём вердикта", "amber"] }
              ],
              "next": "audit_result"
            }
          ]
        },
        {
          ...
          "icon": "📃",
          "title": "Итог проверки",
          "body": "Регулятор прислал заключение. От твоих действий зависит, насколько мягким будет ответ.",
          "choices": [
            {
              "text": "Принять штраф/предписание",
              "desc": "Зависит от того, как готовился",
              "effects": [
                {
                  "roll": {
                    "chance": 0.65,
                    "roleBonus": { "role": "lawyer", "add": 0.2 },
                    "success": [
                      { "rep": 3 },
                      { "log": ["📃 ЦБ: замечаний нет — рейтинг устойчивости вырос", "green"] },
                      { "notify": ["🏛 Проверка пройдена чисто", "success"] }
                    ],
                    "fail": [
                      { "money": -180000 },
                      { "rep": -4 },
                      { "log": ["📃 ЦБ: предписание + штраф 180К", "red"] }
                    ]
                  }
                }
              ],
    ...
    {
      "id": "vip_client_pivot",
      "name": "VIP-клиент уходит к конкуренту",
      "trigger": { "minMonth": 7, "maxMonth": 26, "minRep": 50, "chance": 0.5 },
      "stageDelayMonths": 1,
      "cooldown": 8,
      "stages": [
        {
          "id": "leak",
          "icon": "💼",
          "title": "Утечка от менеджера счёта",
          "body": "Менеджер слил инфу: крупный private-клиент рассматривает переход к «Альфа-Капитал». Решение нужно быстро.",
          "choices": [
            {
              "text": "Запустить персональный пакет (−200 000 ₽)",
              "desc": "Дорогая кастомизация",
              "effects": [
                { "money": -200000 },
                { "rep": 5 },
                { "nudgeAll": 6 },
                { "log": ["💼 Персональный пакет ушёл VIP — клиент остался", "green"] }
              ],
              "end": true
            },
            {
              "text": "Сделать звонок руководителем",
              "desc": "Бесплатно, но эффект непредсказуемый",
              "effects": [
                {
                  "roll": {
                    "chance": 0.5,
                    ...
                    "success": [
                      { "rep": 3 },
                      { "log": ["💼 Звонок отыграл — VIP остался", "teal"] }
                    ],
                    "fail": [
                      { "client": { "sel": "biggest", "op": "rating", "val": -25 } },
                      { "rep": -3 },
                      { "log": ["💼 VIP ушёл к «Альфе» — крупный счёт потерян", "red"] }
                    ]
                  }
                }
              ],
    ...
    },
    {
      ...
      "trigger": { "minMonth": 6, "maxMonth": 26, "minPortfolio": 2, "chance": 0.40 },
      "stageDelayMonths": 1,
      "cooldown": 8,
      "stages": [
        {
          "id": "leak",
          "icon": "🚨",
          "title": "Утечка в Telegram-канале",
          "body": "В канале «Бабкорус» опубликован дамп: ~12К записей клиентских карт. Похоже на инсайдера. Журналисты уже звонят. Что делать?",
          "choices": [
            {
              "text": "Признать публично + компенсация",
              "desc": "Дорого, но честно. ЦБ оценит, рынок простит со временем.",
              "effects": [
                { "log": ["🚨 Утечка: банк выпустил публичное заявление", "amber"] }
              ],
              "next": "apology"
            },
            {
              "text": "Молчать, давить на «технический сбой»",
              "desc": "Экономия сейчас, но если ЦБ узнает — будут проблемы",
              "effects": [
                { "log": ["🚨 Утечка: пресс-служба отрицает факт", "purple"] }
              ],
              "next": "silence"
            }
          ]
        },
        {
          "id": "apology",
          "icon": "📢",
          "title": "Открытое заявление",
          "body": "CEO лично выступил, выплачены компенсации, наняты внешние аудиторы безопасности. Карта репутации:",
          "choices": [
            {
              "text": "Закрыть инцидент",
              "desc": "−500К, но +5 реп. и + к доверию",
              "effects": [
                { "money": -500000 },
                { "rep": 5 },
                { "nudgeAll": 4 },
                { "log": ["📢 Утечка закрыта открыто: −500К, +5 реп.", "teal"] },
                { "notify": ["✅ Кризис коммуникации отыгран", "success"] }
              ],
              "end": true
            }
          ]
        },
        {
          "id": "silence",
          "icon": "🤐",
          "title": "Замолчать удалось — но надолго ли?",
          "body": "СМИ потеряли интерес через неделю. Но в ЦБ начали проверку — нашли источник?",
          "choices": [
            {
              "text": "Положиться на юристов",
              "desc": "Ставка на бэк-офис: успех — сэкономили, провал — штраф ЦБ",
              "effects": [
                {
                  "roll": {
                    "chance": 0.45,
                    "roleBonus": { "role": "lawyer", "add": 0.25 },
                    "success": [
                      { "rep": -2 },
                      { "log": ["🤐 Проверка ЦБ закрыта без штрафа — обошлись", "teal"] }
                    ],
                    "fail": [
                      { "money": -800000 },
                      { "rep": -10 },
                      { "nudgeAll": -12 },
                      { "log": ["⚠️ ЦБ выписал штраф 800К + реп −10", "red"] },
                      { "notify": ["⚠️ Штраф ЦБ + клиенты уходят", "danger"] }
                    ]
                  }
                }
              ],
    ...
    },
    {
      ...
      "name": "Заход в регион — открытие филиала",
      "trigger": { "minMonth": 12, "maxMonth": 30, "requires": "regulator_call", "minRep": 40, "chance": 0.5 },
      "stageDelayMonths": 2,
      "cooldown": 8,
      "stages": [
        {
          "id": "proposal",
          "icon": "🗺",
          "title": "Предложение от региональной администрации",
          "body": "Губернатор области X предлагает: открываете филиал в регионе — получаете льготы по налогам, обслуживаете муниципалитет. Но рынок там мелкий, маржа низкая, риск — нулевая отбивка.",
          "choices": [
            {
              "text": "Открыть филиал — за стратегию роста",
              "desc": "Игра вдолгую: успех — широкая база, провал — деньги в трубу",
              "effects": [
                { "money": -400000 },
                { "log": ["🗺 Открыли филиал в регионе X: −400К на запуск", "purple"] }
              ],
              "next": "open_result"
            },
            {
              "text": "Подождать — пусть конкурент проверит",
              "desc": "Без затрат, но без upside",
              "effects": [
                { "rep": 1 },
                { "log": ["🗺 Решили подождать — конкурент пошёл вперёд", "amber"] }
              ],
              "next": "wait_result"
            }
          ]
        },
        {
          "id": "open_result",
          "icon": "🏪",
          "title": "Через полгода: как зашёл регион?",
          "body": "Главный вопрос — насколько прокачаны процессы. Если команда вытянула — регион стал стабильной касой.",
          "choices": [
            {
              "text": "Подвести итог",
              "desc": "Ставка на HR (выстроить локальную команду)",
              "effects": [
                {
                  "roll": {
                    "chance": 0.55,
                    "roleBonus": { "role": "hr", "add": 0.20 },
                    "success": [
                      { "money": 800000 },
                      { "rep": 6 },
                      { "gAdd": { "portfolio": 10, "caseScoutBonus": 1 } },
                      { "log": ["🏪 Филиал работает: +800К, +6 реп., +1 лид/мес.", "green"] },
                      { "notify": ["🏪 Регион X прибыльный — масштабируем", "success"] }
                    ],
                    "fail": [
                      { "money": -250000 },
                      { "rep": -4 },
                      { "log": ["🏪 Филиал не отбивается: −250К ежемесячно, −4 реп.", "red"] }
                    ]
                  }
                }
              ],
        ...
        },
        {
          "id": "wait_result",
          "icon": "🥶",
          "title": "Конкурент захватил регион",
          "body": "Соседний банк зашёл первым, подписал муниципалитет, выкупил местного игрока. Окно закрылось.",
          "choices": [
            {
              "text": "Сосредоточиться на столице",
              "desc": "Без потерь, но без региона",
              "effects": [
                { "rep": 2 },
                { "log": ["🥶 Регион упущен — фокус на столице сохранён", "teal"] }
              ],
              "end": true
            }
          ]
