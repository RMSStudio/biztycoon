...
(function () {
  ...
  const BASE_WIN      = 150;
  const BASE_LOSS     = 40;
  // §Ф.7 — «Вступительный ран» (заскриптованное поражение). Голый ран (пока не
  // открыт ни один узел) нельзя выиграть: без команды/инструментов студию не
  // удержать. Чтобы это не тянулось (и не «уходило в плюс по чуть-чуть»), такой
  // ран принудительно завершается поражением к INTRO_DEADLINE месяцу — как
  // вступительная миссия / очень сильный «босс» в начале. Оформляется ободряюще.
  const INTRO_DEADLINE = 14;   // мес. — потолок под-экипированного рана (тюнинг §15.6)
  // §Ф.7 — «операционная перегрузка» под-экипированного рана: чтобы бюджет не полз
  // в плюс по чуть-чуть, каждый месяц списывается растущая сумма (мало систем/рук →
  // фаундер не тянет объём: срывы, штрафы, простой). При старте 1M и overhead 35k
  // бюджет закономерно уходит в минус, а таймер INTRO_DEADLINE — страховка. §15.6.
  const INTRO_STRAIN = 7500;   // ₽/мес × номер месяца (линейный рост, × тейпер)
  // §Ф.7 — порог «жизнеспособного ядра»: пока открыто МЕНЬШЕ INTRO_CORE узлов дерева,
  // ран считается вступительным (утечка+потолок работают, слабея с каждым открытием).
  // Открыл ≥ INTRO_CORE — ран «настоящий»: ни утечки, ни потолка. Фикс: раньше порог
  // был 1 узел (opened===0), и 1 найм уже позволял «уходить в плюс и не проигрывать».
  const INTRO_CORE = 4;        // сколько узлов нужно, чтобы ран перестал быть вступительным
  ...
  // ── §Ф.7: «вступительный» (под-экипированный) ран ────────────────────
  // Вступительный ран = режим активен и открыто МЕНЬШЕ INTRO_CORE узлов дерева.
  // Пока систем мало — студию не вывести (даже 1 найм не спасает: нет процесса/
  // скаута/рынка). Утечка+потолок+фрейминг работают, слабея с каждым открытием.
  // Открыл ≥ INTRO_CORE — ран «настоящий» (ни утечки, ни потолка). Признак единый.
  function isBareIntro(g) {
    if (!isActive() || !g) return false;
    return _state().opened.length < INTRO_CORE;
  }

  // Операционная перегрузка: растущее списание × тейпер по числу открытых узлов —
  // opened 0 → полная, 1 → ×0.75, 2 → ×0.5, 3 → ×0.25, ≥INTRO_CORE → 0. Чистая функция.
  function introBleed(g) {
    if (!isBareIntro(g)) return 0;
    const opened = _state().opened.length;
    const taper  = Math.max(0, (INTRO_CORE - opened) / INTRO_CORE);
    const month  = Math.max(1, (g.month || 0));
    return Math.round(INTRO_STRAIN * month * taper);
  }

  // Заскриптованный потолок (страховка): под-экипированный ран, доживший до
  // INTRO_DEADLINE, принудительно проигрывается — гарантия, что нельзя «коастить»
  // в плюс на 1 найме + разовых, пока не собран жизнеспособный core (≥ INTRO_CORE).
  ...
  window.Unlocks = {
    ...
    TUNING: { BASE_WIN, BASE_LOSS, STAGE_EXP, FIRST_WIN_EXP, INTRO_DEADLINE, INTRO_STRAIN, INTRO_CORE },   // ручки §15.6 (для тестов/сима)
