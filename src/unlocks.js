...
(function () {
  ...
  // по чуть-чуть, каждый месяц голого рана списывается растущая сумма (нет
  // команды → фаундер не тянет объём: срывы, штрафы, простой). При старте 1M и
  // overhead 35k бюджет закономерно уходит в минус к ~12 месяцу (банкротство
  // движка), а таймер INTRO_DEADLINE остаётся страховкой. Ручка §15.6.
  const INTRO_STRAIN = 7500;   // ₽/мес × номер месяца (линейный рост)
  ...
  // Голый ран = режим активен и НИ ОДИН узел «Дерева открытий» ещё не открыт
  // (opened пуст) — это самый первый ран(ы), пока не потрачена экспертиза.
  // Его нельзя вывести (нет ни команды, ни инструментов) — вступительная «миссия».
  // Как только открыт хоть один узел — ран становится «настоящей» попыткой.
  // Признак единый для утечки бюджета, скриптового потолка и фрейминга.
  // (Стадию НЕ проверяем: фреш-ран стартует со стадии 1, а не 0.)
  function isBareIntro(g) {
    if (!isActive() || !g) return false;
    return _state().opened.length === 0;
  }
