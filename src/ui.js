...
function buildDashboard(won) {
  ...
  if (!won && typeof G !== 'undefined' && G._scriptedIntro) {
    ...
    document.getElementById('r-sub').textContent =
      'Так и должно быть. Пока систем открыто мало, студию не удержать — это начало пути, а не провал. ' +
      'Ты нащупал, как всё устроено, и заработал экспертизу — открой ещё модули в «Дереве открытий», и следующий ран будет глубже.';
  }
